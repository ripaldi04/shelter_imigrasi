...
@section('content')
            ...
            <table class="w-full table-auto text-sm border rounded">
                <thead class="bg-gray-100 text-left">
                    <tr>
                        ...
                        <th class="px-4 py-2">Jenis Kenaikan Pangkat</th>
                        <th class="px-4 py-2">Tanggal TMT</th>
    ...
    <div id="formModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-3xl">
            <h3 class="text-lg font-semibold mb-4">Tambah Data Pangkat</h3>
            <form action="/pangkat/store" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="text-sm">Golongan</label>
                        <input type="text" name="golongan" class="w-full border rounded px-3 py-2" required />
                    </div>
                    <div>
                        <label class="text-sm">Status Kepegawaian</label>
                        <input type="text" name="status_kepegawaian" class="w-full border rounded px-3 py-2" required />
                    </div>
                    <div>
                        <label class="text-sm">Jenis Kenaikan Pangkat</label>
                        <input type="text" name="jenis_kenaikan" class="w-full border rounded px-3 py-2" />
                    </div>
                    <div>
                        <label class="text-sm">Tanggal TMT</label>
                        <input type="date" name="tmt" class="w-full border rounded px-3 py-2" required />
                    </div>
                    <div>
                        <label class="text-sm">Masa Kerja</label>
                        <input type="text" name="masa_kerja" class="w-full border rounded px-3 py-2" />
                    </div>
                    <div>
